...
class SDFLoss(nn.Module):
    ...
    def check_overlap(self, bbox1, bbox2):
        ...
        # print('bbox2', bbox2[:, 0])
        # pdb.set_trace()
        #print(bbox1[0,0] > bbox2[1,0] or bbox2[0,0] > bbox1[1,0])
    ...
    def forward(self, vertices, translation, scale_factor=0.2):
        ...
        isolated_boxes = self.filter_isolated_boxes(boxes)
        isolated_boxes = isolated_boxes.type(torch.bool)
        overlapping_boxes = ~isolated_boxes
        ...
        for i in range(valid_people):
            ...
            phi_val = nn.functional.grid_sample(phi[i][None, None], vertices_grid, align_corners=True).view(valid_people, -1)
